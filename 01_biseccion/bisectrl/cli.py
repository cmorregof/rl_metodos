"""Punto de entrada: `python -m bisectrl` o `bisectrl`."""
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ap = argparse.ArgumentParser(
        prog="bisectrl",
        description="Mira en vivo cómo un agente de RL aprende la bisección y demuestra su convergencia.",
    )
    ap.add_argument("--episodes1", type=int, default=3000, help="generaciones de la fase 1 (bisección)")
    ap.add_argument("--episodes2", type=int, default=3000, help="generaciones de la fase 2 (demostración)")
    ap.add_argument("--seed", type=int, default=0)
    ap.add_argument("--tol", type=float, default=1e-3, help="tolerancia del intervalo")
...
def main(argv: list[str] | None = None) -> int:
    ...
    cfg = Config(episodes_bisection=args.episodes1, episodes_proof=args.episodes2, seed=args.seed, tol=args.tol)
    tr = Trainer(cfg)
    console = Console()
