"""Punto de entrada: `python -m secantrl` o `secantrl`."""
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ap = argparse.ArgumentParser(
        prog="secantrl",
        description="Mira en vivo cómo un agente de RL aprende la secante con salvaguardas y demuestra que su orden de convergencia es φ = (1 + √5)/2.",
    )
    ap.add_argument("--episodes1", type=int, default=3000, help="generaciones de la fase 1 (secante)")
...
def main(argv: list[str] | None = None) -> int:
    ...
    cfg = Config(episodes_secant=args.episodes1, episodes_proof=args.episodes2, seed=args.seed, tol=args.tol)
    tr = Trainer(cfg)
    console = Console()
