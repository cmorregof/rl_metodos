...
from .env_secant import ACTION_SHORT, BISECT, FALSI, N_ACTIONS, PHI, RATIO_LABELS, SEC, theoretical_action
from .proof_kb import MIN_PROOF_LENGTH, STEPS
from .trainer import ALL_STATES, Trainer

SPARK = "▁▂▃▄▅▆▇█"
ACTION_STYLE = {SEC: "yellow", FALSI: "magenta", BISECT: "blue"}
...
# Fase 1
# ---------------------------------------------------------------------------
def plot_secant(tr: Trainer, width: int = 46, height: int = 8) -> Text:
    """f con la raíz ◆, el corchete ⟨ ⟩, los iterados ○● sobre el eje y la secante por los dos últimos puntos."""
    env = tr.env1
    p = env.problem
    finite = [x for x in env.history if math.isfinite(x)]
    span = max((p.hi - p.lo) * 0.6, 0.5)
    mid = 0.5 * (p.lo + p.hi)
    lo_x, hi_x = mid - span, mid + span
    ...
    # secante por los dos últimos puntos (la propuesta del siguiente paso)
    if math.isfinite(env.x) and math.isfinite(env.x_prev) and math.isfinite(env.fx) and math.isfinite(env.fx_prev) and env.x != env.x_prev:
        slope = (env.fx - env.fx_prev) / (env.x - env.x_prev)
        for c, x in enumerate(xs):
            r = row_of(env.fx + slope * (x - env.x))
            if r is not None:
    ...
    if z is not None:
        for x, ch in ((env.lo, "⟨"), (env.hi, "⟩")):
            c = col_of(x)
            if 0 <= c < width:
                grid[z][c], styles[z][c] = ch, "bold blue"
...
def error_bars(tr: Trainer, width: int = 46, rows: int = 5) -> Text:
    """Últimas iteraciones: error |xₙ − r| en escala log y cómo se generó cada punto (sec / rf / bis)."""
    env = tr.env1
    t = Text(no_wrap=True, overflow="ellipsis")
    hist = env.history[-rows:]
    acts = ([None, None] + env.action_history)[-rows:]
    start = len(env.history) - len(hist)
    for i, (x, a) in enumerate(zip(hist, acts)):
        ...
        if a is not None:
            t.append(f" {ACTION_SHORT[a]:>3}", style=ACTION_STYLE[a])
        else:
            t.append(" ⟨⟩ ", style="dim")
        if i < len(hist) - 1:
            t.append("\n")
...
def phase1_panel(tr: Trainer, height: int) -> Panel:
    ...
    status = {
        ...
        "undefined": "[bold red]✘ secante indefinida (f(xₙ) = f(xₙ₋₁))[/]",
    ...
    inside_txt = "dentro" if env.secant_inside else "[red]fuera[/]"
    order = env.observed_order()
    order_txt = f"  orden≈{order:.2f}" if order == order and 0 < order < 6 else ""
    head = Text.from_markup(
        f"[bold]f(x) = {env.problem.name}[/]   corchete ⟨{env.lo:.4g}, {env.hi:.4g}⟩   raíz ◆ {env.problem.root:.4f}\n"
        f"it {env.n_iter:>2} ({env.n_evals} eval)  ρ = {rho_txt}  secante {inside_txt}  acción: {ACTION_SHORT[tr.last_action1]}{order_txt}   {status}",
        overflow="ellipsis",
    )
    head.no_wrap = True
    body = Group(head, plot_secant(tr, height=plot_h), error_bars(tr, rows=bars_h))
    title = "Fase 1 · aprendiendo la secante con salvaguardas" + ("" if active else " · completada ✔")
...
def law_panel(tr: Trainer) -> Panel:
    ...
    tb.add_row("estado (ρ, secante)", "acción", "Dekker", "Q(sec, rf, bis)")
    for s in ALL_STATES:
        b, i = s
        a = law[s]
        theo = theoretical_action(b, i)
    ...
    med = tr.order_median()
    order_txt = f"{med:.2f}" if med == med else "—"
    foot = Text.from_markup(f"Dekker: [bold]{ok}/{n}[/]   orden observado: [bold]{order_txt}[/] (φ={PHI:.3f})   éxito: ", overflow="ellipsis")
    foot.no_wrap = True
    foot.append(sparkline(tr.success_curve, 14), style="green")
...
def proof_lines(attempts: list[tuple[int, bool]], max_lines: int, show_text: bool = True, tail: bool = True) -> Text:
    ...
    elif len(attempts) > max_lines:
        # cabecera de la demostración, elipsis y siempre la última línea (el ∎)
        shown = attempts[: max_lines - 2] + [None] + attempts[-1:]
        hidden = len(attempts) - (max_lines - 1)
    else:
        shown = list(attempts)
    n = 0
    for item in shown:
        if item is None:
            t.append(f"  … {hidden} pasos más\n", style="dim")
            n += hidden
            continue
        a, ok = item
        st = STEPS[a]
...
def phase2_panel(tr: Trainer, height: int) -> Panel:
    ...
    title = "Fase 2 · aprendiendo a demostrar el orden φ de la secante" + ("" if active else " · completada ✔")
...
def stats_panel(tr: Trainer) -> Panel:
    ...
    tb.add_row("F1 ok", f"[green]{tr.converged_total}[/]", "F1 falló", f"[red]{tr.diverged_total + tr.timeout_total}[/]")
...
def log_panel(tr: Trainer, n: int) -> Panel:
    ...
    for line in tr.log[-n:]:
        style = "bold green" if any(k in line for k in ("mínima", "Dekker", "φ", "completa!", "refugio")) else ""
        t.append(line + "\n", style=style)
...
def header(tr: Trainer) -> Panel:
    txt = Text.from_markup(
        "[bold white]SecantRL[/] · un agente aprende la secante con salvaguardas y a demostrar su orden φ = (1+√5)/2   "
