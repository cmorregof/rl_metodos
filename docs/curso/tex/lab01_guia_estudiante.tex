...
Ahora inventa tú los puntos. Los tres números de la recompensa se pueden fijar desde la terminal (los originales son $-10$, $-1$ y $0$):
\begin{orden}
.venv\Scripts\python ..\docs\curso\lab01\experimentos.py recompensa --perder-raiz 0 --paso -1 --converger 0
\end{orden}
\begin{preguntas}
\pregunta{h}{Propón una recompensa que \textbf{parezca} razonable y que el agente pueda explotar sin resolver el problema. Antes de correrla, escribe qué crees que hará. Luego pruébala con la orden de arriba y anota qué hizo de verdad (convergencias, raíces perdidas, $\lambda^*$).}
\end{preguntas}
...
\end{preguntas}
Cuando lo tengas en papel, compáralo con el que dibuja el programa (sale como un árbol desde \qed\ hacia las hipótesis):
\begin{orden}
