...
Antes de este laboratorio lee \emph{La terminal desde cero}. Los primeros 30 minutos de la sesión son para hacer lo que dice esa guía, con ayuda. Todas las órdenes de abajo se escriben dentro de la carpeta \codigo{01\_biseccion} y empiezan por \codigo{.venv\textbackslash Scripts\textbackslash python} (en Mac, \codigo{.venv/bin/python}).
...
\pregunta{e}{Corre otra vez con \codigo{--seed 2}. ¿Cambian las generaciones de los hitos? ¿Cambia lo aprendido?}
\end{preguntas}
...
\begin{enumerate}
\item La tabla de hitos con dos semillas.
\item Respuestas a (a)–(n). Una respuesta correcta suele caber en dos líneas.
\item Un párrafo: \emph{si tuvieras que escribir el grafo de otro teorema del curso, ¿cuál elegirías y cuáles serían sus hipótesis y su \qed?} Esto es la semilla del proyecto final.
\end{enumerate}
...
\subsection*{Hacia dónde va esto}

En las próximas sesiones el agente aprenderá punto fijo, Taylor, Newton, interpolación, integración, sistemas lineales y ecuaciones diferenciales con la misma receta. Luego cambiará el juego: \textbf{tú} escribirás el grafo de un teorema que no está en el repositorio y diseñarás las recompensas de un método. Y al final veremos qué pasa cuando el verificador no es un grafo escrito a mano sino un programa que certifica cada propuesta: ahí el agente sí puede encontrar cosas que nadie le dijo.
\end{document}
