...
\pregunta{h}{Respuestas típicas buenas: «premiar por cada evaluación de $f$» (evaluaría infinito), «premiar que $|f(x)|$ sea pequeño» (se estanca donde $f$ es plana), «premiar solo al final» (aprende más lento pero no se puede engañar). Cualquier respuesta que identifique \emph{qué haría el agente} vale. Medido con los flags: \codigo{--perder-raiz 0} da 0 convergencias (el caso B); \codigo{--paso 0 --converger 50} sigue convergiendo (el bono final no cambia nada porque la cabeza de corte es miope) y puede elegir $\lambda=0{,}4$ por ruido; y \textbf{\codigo{--paso 1}, premiar cada evaluación, no se explota}: el agente sigue convergiendo al 100\,\%. Es un matiz honesto para la discusión: las dos cabezas de la fase 1 usan $\gamma=0$, solo miran la recompensa inmediata y no pueden «planear» alargar el episodio. Un agente con $\gamma>0$ sí lo explotaría. La lección es que lo que se puede explotar depende de la recompensa \emph{y} del agente.}
